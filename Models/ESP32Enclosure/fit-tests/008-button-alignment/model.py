"""Actual revision013 button alignment on a low, open production fit frame.
...
def build(params):
    ...
    for name,shape in parts.items():
        if not shape.is_valid or len(shape.solids())!=1 or abs(shape.bounding_box().min.Z)>.001:
            raise ValueError(f'{name} must be one solid in production print orientation onZ0')
    ...
    tips={name:{'center_local_xy_mm':[data['params']['tip_x'],data['params']['tip_y']],
          ...
          for name,data in full['buttons'].items()}
    slider_parts={n:s for n,s in parts.items() if n.endswith('_slider')}
    lid_plane=full['reference_measurements']['pcb_top_z']+p['component_clearance']
    return {
        'title':'Measured button alignment / test008',
        ...
        'slider_only_parts':slider_parts,
        'measurements':{
            'source_revision':'013-measured-buttons',
            'coordinate_system':'Case print frame: floorZ0, terminals+Y, USB/buttons-X; bench press is-Z, mounted press is world+Z',
            ...
            'two_sliders_cad_volume_cm3':sum(float(s.volume) for s in slider_parts.values())/1000,
        ...
        'notes':[
            ...
            f'Place the49x70x1 mm board on the ledges atZ{info["board_bottom_local_z_mm"]:g}; align by equal side/end gaps before checking the nominal measured tip centers. The board can move+/-{xplay:g} mm inX and+/-0.4 mm inY, so verify contact at its movement limits too.',
            ...
            f'The omitted lid inner face is localZ{lid_plane:g}. Measure the adjusted contact head against that plane or close the real lid during the two-slider trial; no fictitious component height or extra printed gauge is included.',
        ],
    }
