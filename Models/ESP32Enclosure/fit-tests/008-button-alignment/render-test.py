...
def grouped(shapes):
    ...
    groups={'test_frame':[], 'four_pcb_clamps':[], 'frames_and_keepers':[], 'two_measured_sliders':[]}
    for name,shape in shapes.items():
        key=('test_frame' if name=='button_fit_frame' else 'four_pcb_clamps' if name.startswith('pcb_clamp_')
             else 'two_measured_sliders' if name.endswith('_slider') else 'frames_and_keepers')
...
def main():
    ...
    preview.render(grouped(model['assembly']),output/'preview.png',title='Measured button alignment / quick trial',
    ...
    preview.render(print_layout(model['slider_only_parts']),output/'sliders-only.png',title='Start here / two replacement sliders',
                   subtitle='Reuse verified revision 012 parts or test 007 frame · local arm supports required',elevation=52,azimuth=-115)
    preview.PALETTE+=['#679869']
    preview.render(grouped(model['assembly'])|{'reference_pcb':model['reference_pcb']},output/'fit-reference.png',
                   title='Actual PCB position / measured button layout',
                   subtitle='49 × 70 × 1 mm board · tip inset 3.3 mm · centers 30/16 mm from bottom',elevation=70,azimuth=-125)
