"""Revision 012 base: independent board-fit clearance and deeper PCB capture shelves.

Assembly coordinates: board centered in XY, +Y is terminal/cable end, -X is
USB/DC access side, +Z points from the floor toward the open lid in case-local print coordinates. Body floor bottom is Z=0.
...
DEFAULTS = {
    ...
    "board_edge_overlap": 1.3, "board_vertical_play": 0.2,
...
def build_console(params: dict) -> dict:
    ...
    if any(v <= 0 for k, v in p.items() if k not in ("left_access_start_above_pcb", "board_fit_width_extra")):
    ...
    if not 0.0 <= p["board_fit_width_extra"] <= 1.0:
        raise ValueError("Additional total board-fit width must be0..1 mm")
    if not 0.4 <= p["board_edge_overlap"] <= 1.5:
        raise ValueError("PCB retention overlap must be0.4..1.5 mm")
    if p["board_edge_overlap"] <= 0.4+p["board_fit_width_extra"]/2:
        raise ValueError("PCB shelves and clamps must retain positive overlap after all lateral play")
    ...
    # The board remains measured size. Structural fit width moves the walls,
    # locating shoulders and fasteners, without moving the board-relative tips
    # or reducing the requested shelf overlap beneath the actual substrate.
    fit_extra = p["board_fit_width_extra"]
    rw = bw+fit_extra
    side_play = 0.4+fit_extra/2
    wall, floor = p["wall"], p["floor"]
    cw, cl = rw + 2 * p["side_margin"], bl + 2 * p["end_margin"]
    ...
    clamp_x = rw / 2 + p["pcb_clamp_screw_offset_from_edge"]
    clamp_y = p["clamp_y"]
    ...
    if clamp_x-boss_r < rw/2+0.4-1e-8:
    ...
    if clamp_x-rw/2-0.4-insert_r < 2.0-1e-8:
    ...
    return {
        "title": "ESP32 / board-fit clearance and capture shelves · revision012 base",
        "parts": parts,
        "assembly": assembly,
        ...
        "measurements": {
            ...
            "board_retention": {
                "measured_pcb_width_mm":bw,
                "additional_total_fit_width_mm":fit_extra,
                "structural_width_mm":rw,
                "shoulder_inner_abs_x_mm":rw/2+0.4,
                "shoulder_gap_mm":rw+0.8,
                "centered_side_clearance_each_mm":side_play,
                "shelf_inner_abs_x_mm":clamp_inner,
                "shelf_inner_gap_mm":2*clamp_inner,
                "shelf_and_clamp_outer_abs_x_mm":clamp_outer,
                "shelf_and_clamp_total_width_mm":clamp_width,
                "shelf_depth_from_shoulder_mm":rw/2+0.4-clamp_inner,
                "nominal_overlap_over_measured_board_mm":p["board_edge_overlap"],
                "minimum_overlap_after_lateral_play_mm":p["board_edge_overlap"]-side_play,
                "clamp_screw_abs_x_mm":clamp_x,
                "pcb_thickness_mm":p["pcb_thickness"],
                "vertical_play_mm":p["board_vertical_play"],
                "capture_slot_height_mm":p["pcb_thickness"]+p["board_vertical_play"],
                "board_bottom_local_z_mm":board_bottom,
                "board_top_local_z_mm":board_top,
                "clamp_underside_local_z_mm":clamp_z,
                "full_width_change_from_011_mm":fit_extra,
                "nominal_overlap_change_from_011_mm":p["board_edge_overlap"]-0.8,
                "fit_status":"Provisional allowance/deeper bare-edge capture; print and test the current board coupon",
            },
            "corner_bosses": {
        ...
        "notes": [
            "Revision012 adds independent board-fit width while extending both lower shelves and matching top clamps over the measured substrate. Older revisions remain preserved.",
            ...
            f"Measured PCB substrate is {bw:g}x{bl:g} mm; the structural fit datum is {rw:g} mm wide. These are separate from the populated electronics span.",
            ...
            f"PCB thickness defaults to the measured {p['pcb_thickness']:g} mm; underside solder allowance is a placement choice that still needs a physical clearance check.",
            ...
            f"Side shoulders allow {side_play:g} mm centered board movement in X; low end stops retain0.4 mm in Y. Minimum shelf/clamp overlap after full lateral play is {p['board_edge_overlap']-side_play:g} mm; verify bare edges above and below the PCB.",
...
BUTTON_DEFAULTS = {
    "reset_button_from_bottom": 30.0, "boot_button_from_bottom": 16.0,
    "button_inset_from_left_pcb_edge": 3.3,
    "reset_housing_y": 4.0, "boot_housing_y": -21.0,
    "button_arm_elbow_inset": 4.5,
    "button_contact_collar_flat_offset": 2.9,
...
def build(params: dict) -> dict:
    ...
    face_x = -ow/2 - 3.6
    # Switch contact stays relative to the actual PCB. The widened wall
    # moves the guide outward, so the slider arm length grows automatically.
    ...
    # Measured switch positions and exterior guide positions are independent.
    # Keep the established mounts while an inboard dogleg reaches each switch.
    tip_ys = [p["reset_button_from_bottom"]-p["pcb_length"]/2,
              p["boot_button_from_bottom"]-p["pcb_length"]/2]
    if not 10 <= tip_ys[0]-tip_ys[1] <= 20:
        raise ValueError("Switch separation must stay 10..20 mm for these contact bosses")
    if any(abs(y)+4.5 > p["pcb_length"]/2 for y in tip_ys):
        raise ValueError("Contact boss must stay within the PCB length")
    centers = [p["reset_housing_y"], p["boot_housing_y"]]
    ...
    result["hardware"] = {}
    result["buttons"] = {}
    result["usb_relief_references_local"] = {}
    for name, center, tip_y, switch_h, screw_length in (
        ("reset", centers[0], tip_ys[0], p["reset_switch_height_above_pcb"], p["reset_contact_screw_length"]),
        ("boot", centers[1], tip_ys[1], p["boot_switch_height_above_pcb"], p["boot_contact_screw_length"]),
    ):
        bp = {"mounting_face_x": face_x, "housing_y": center,
              ...
              "arm_elbow_inset": p["button_arm_elbow_inset"],
              "collar_flat_offset": p["button_contact_collar_flat_offset"],
              "collar_relief_direction": -1 if name == "reset" else 1,
              "usb_board_left_x": -p["pcb_width"]/2,
              "usb_board_bottom_y": -p["pcb_length"]/2,
              "usb_board_top_z": board_top,
              **{key: p[key] for key in BUTTON_DEFAULTS if key.startswith("usb_")},
        ...
        for key, shape in button["usb_relief_local"].items():
            reference_key = key if key in ("plug_nominal", "plug_clearance") else f"{name}_{key}"
            result["usb_relief_references_local"][reference_key] = shape
    ...
    usb = result["buttons"]["reset"]["measurements"]["usb_clearance"]
    result["measurements"]["usb_clearance"] = {
        "measurement_record": "measurements/2026-09-13-usb-connector.json",
        "fit_test": "fit-tests/009-usb-clearance",
        "status": "Measured nominal plug envelope relieved; physical fit and PCB-placement limits pending",
        "parameters": {key: p[key] for key in BUTTON_DEFAULTS if key.startswith("usb_")},
        "measurement_chart_mm": {"A_width": p["usb_plug_width"],
            "B_top_above_pcb": p["usb_plug_underside_above_pcb"]+p["usb_plug_thickness"],
            "C_outboard_projection": p["usb_plug_projection"], "D_underside_above_pcb": p["usb_plug_underside_above_pcb"],
            "E_center_from_bottom": p["usb_center_from_bottom"], "F_cable_diameter": p["usb_cable_diameter"]},
        "plug_thickness_mm": p["usb_plug_thickness"],
        "raw_user_reply_mm": {"A": 10.0, "B": 6.0, "C": 20.0, "D": 9.0, "E": 23.0, "F": 3.5},
        "raw_reply_clarification": "The user clarified raw B=6 as plug thickness. Original diagram B means top height above PCB: D9 + thickness6 = B15. Current diagram fields above are normalized from the active parameters.",
        "datum": "Case-local centered measured PCB: X left=-width/2, Y bottom=-length/2, Z top=floor+solder_clearance+pcb_thickness",
        "board_left_x_mm": -p["pcb_width"]/2,
        "board_bottom_y_mm": -p["pcb_length"]/2,
        "board_top_z_mm": board_top,
        "plug_top_above_pcb_derived_mm": p["usb_plug_underside_above_pcb"]+p["usb_plug_thickness"],
        "nominal_plug_bounds_local_mm": usb["nominal_plug_bounds_local_mm"],
        "clearance_plug_bounds_local_mm": usb["clearance_plug_bounds_local_mm"],
        "slider_relief_bounds_at_rest_local_mm": usb["slider_relief_bounds_at_rest_local_mm"],
        "slider_motion_sweep_bounds_at_rest_local_mm": usb["slider_motion_sweep_bounds_at_rest_local_mm"],
        "slider_loading_relief_open_below": True,
        "cable_axis_local_yz_provisional_mm": [p["usb_center_from_bottom"]-p["pcb_length"]/2,
            board_top+p["usb_plug_underside_above_pcb"]+p["usb_plug_thickness"]/2],
        "cable_axis_status": "Provisional centered on plug section; only cable diameter is measured",
        "plug_envelope_assumption": "Maximum measured width covers the full outboard projection; taper and strain-relief profile are unmeasured",
        "fit_clearance_status": "FDM allowance is provisional, independent of measured USB dimensions",
        "board_placement_status": "Relief uses nominal centered board; existing fit gap and all board-retention geometry are preserved",
        "minimum_wall_to_insert_envelope_mm": min(b["measurements"]["usb_clearance"]["minimum_wall_to_insert_envelope_mm"] for b in result["buttons"].values()),
        "buttons": {name: b["measurements"]["usb_clearance"] for name, b in result["buttons"].items()},
        "physical_fit_tested": False,
    }
    result["title"] = "ESP32 / measured USB clearance in button cartridges · revision 014"
    result["notes"] = [n for n in result["notes"] if not n.startswith("Revision 002")]
    result["notes"].extend([
        "Revision 014 adds local USB relief to the RESET and BOOT sliders and BOOT frame while retaining the existing M3 heat-set hardware and every board/mount datum.",
        "Button XY uses the supplied PCB edge measurements; switch height, travel, board position and physical fit remain provisional.",
        "The USB envelope uses measured width, thickness, outboard projection, underside height and center position. Its top is derived from underside plus thickness. Fit the USB plug before the cartridges and verify actual clearance, including PCB movement within its unchanged fit gap.",
