...
def main():
    ...
    reuse_frame_path=args.reuse_frame/'parts/button_fit_frame.step'
    if not reuse_frame_path.exists():reuse_frame_path=args.reuse_frame/'parts/board_fit_frame.step'
    same(m['parts']['button_fit_frame'],import_step(reuse_frame_path),'Reuse prior fit frame')
    for name in (n for n in m['parts'] if n.startswith('pcb_clamp_')):
        same(m['parts'][name],import_step(args.reuse_frame/'parts'/f'{name}.step'),name+' reuse prior fit clamp')
    unchanged=[]
    for name,shape in full['parts'].items():
        if name in ('reset_slider','boot_slider','boot_frame'):continue
        same(shape,import_step(args.reuse_enclosure/'parts'/f'{name}.step'),name+' reuse revision013')
        unchanged.append(name)
    require(len(unchanged)==12,'Expected twelve unchanged full-enclosure parts')
    reused_jig=[]
    for name,shape in m['parts'].items():
        if name in ('reset_slider','boot_slider','boot_frame'):continue
        prior=(reuse_frame_path if name=='button_fit_frame' else args.reuse_frame/'parts'/f'{name}.step')
        if prior.exists():
            same(shape,import_step(prior),name+' reused jig part')
            reused_jig.append(name)
    require(len(reused_jig)==10,'Expected ten unchanged jig parts')
    instances=load_mesh_instances(source/'replacements-only.3mf')
    require(len(instances)==3,'Replacement 3MF must contain exactly three instances')
    expected_layout=print_layout(m['replacement_parts'])
    ...
    require(np.allclose(actual_bounds,expected_bounds,atol=.05,rtol=0),'Replacement layout changed scale or placement')
    actual_volume=sum(abs(float(s.volume)) for _,s in instances)
    require(abs(actual_volume/sum(float(s.volume) for s in expected_layout.values())-1)<.005,'Replacement mesh volume changed')
    measured={}
    ...
    # Independent construction from the user measurements; the body is at localZ9..10.
    for key,value in {'usb_plug_width':10,'usb_plug_thickness':6,'usb_plug_projection':20,
                      'usb_plug_underside_above_pcb':9,'usb_center_from_bottom':23,
                      'usb_cable_diameter':3.5}.items():
        require(abs(p[key]-value)<1e-8,'Measured USB input changed',parameter=key)
    board_top=m['measurements']['board_top_z_mm']
    plug_bounds=[[-p['pcb_width']/2-20,-p['pcb_length']/2+18,board_top+9],
                 [-p['pcb_width']/2,-p['pcb_length']/2+28,board_top+15]]
    plug=box(*plug_bounds)
    same(plug,m['reference_usb'],'Measured USB box construction')
    outer_bounds=[[plug_bounds[0][0]-20,plug_bounds[0][1],plug_bounds[0][2]],plug_bounds[1]]
    outer_usb=box(*outer_bounds)
    mid_y=-p['pcb_length']/2+23
    cable=Pos(plug_bounds[0][0]-20,mid_y,board_top+12)*Rot(Y=90)*Cylinder(1.75,20,align=(Align.CENTER,Align.CENTER,Align.MIN))
    usb_play_samples=[]
    states=[]
    ...
    for reset_travel,boot_travel in ((0,0),(stroke/2,stroke/2),(stroke,stroke),(stroke,0),(0,stroke)):
        ...
        nominal_hits=hits(plug,printed|hardware)
        require(not nominal_hits,'Centered measured USB box obstructed',travel=[reset_travel,boot_travel],hits=nominal_hits)
        states.append({'buttons':tips,'unexpected_motion_interference':[],
                       'nominal_measured_usb_box_hits_mm3':nominal_hits,
                       'straight_external_plug_approach_hits_mm3':hits(outer_usb,printed|hardware),
                       'measured_cable_outer_segment_hits_mm3':hits(cable,printed|hardware)})
        for dx in (-xp,0,xp):
            for dy in (-.4,0,.4):
                for dz in (0,p['board_vertical_play']/2,p['board_vertical_play']):
                    pose=Pos(dx,dy,dz)
                    plug_hits=hits(pose*plug,printed|hardware)
                    cable_hits=hits(pose*cable,printed|hardware)
                    usb_play_samples.append({'pcb_shift_mm':[dx,dy,dz],
                        'button_travel_mm':[reset_travel,boot_travel],
                        'plug_box_hits_mm3':plug_hits,'cable_segment_hits_mm3':cable_hits})
    stop_contacts={}
    ...
    report={'status':'passed','source_hashes':source_hashes,'checker_sha256':sha(__file__),
            ...
            'exact_production_interfaces':True,'reuse_prior_frame_and_clamps':True,
            'unchanged_revision013_full_parts':unchanged,'unchanged_jig_parts':reused_jig,
            'replacement_parts':['reset_slider','boot_slider','boot_frame'],
            'replacements_only_3mf_instances':3,'measured_button_xy':measured,'board_seating_poses_checked':27,
            'sampled_motion_states':states,'positive_stop_checks':stop_contacts,
            'usb_probe_definition':{'measured_plug_box_local_mm':plug_bounds,
                'straight_external_approach_box_local_mm':outer_bounds,
                'measured_cable_diameter_mm':3.5,'cable_center_yz_mm':[mid_y,board_top+12],
                'actual_dimensions_measured':True,'plug_shape_conservative_box':True,
                'cable_path_assumption':'Straight20mm external segment centered at plug mid-height; actual cable-axis height, bend and taper are unmeasured',
                'physical_insertion_verified':False},
            'usb_board_play_samples':usb_play_samples,
            'usb_board_play_obstructed_samples':sum(bool(x['plug_box_hits_mm3'] or x['cable_segment_hits_mm3']) for x in usb_play_samples),
            'physical_fit_tested':False,'limitations':[
                'Nominal measured-box clearance is required; conservative-box conflicts at PCB-play limits remain reported and unresolved. Fit the actual plug before the cartridges and check travel and locating extremes.',
    ...
    print(json.dumps({'status':'passed','report':str(args.output),'three_part_replacement_reuse_checked':True,
                      'usb_board_play_obstructed_samples':report['usb_board_play_obstructed_samples']}))
