"""Freeze a button-alignment trial and export full and three-part replacement print layouts."""
import argparse
import hashlib
import json
...
def main():
    ...
    export_mesh(print_layout(model['replacement_parts']),output/'replacements-only.3mf')
    ...
    print(json.dumps({'status':report['status'],'output':str(output),'print_parts':len(model['parts']),
                      'replacement_parts':list(model['replacement_parts'])}))
