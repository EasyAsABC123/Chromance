# Quick cable-retention fit test — 003

Print **[print-layout.3mf](print-layout.3mf)** before reprinting the enclosure.
It contains two small parts: a section of revision 009's body with the actual
...
1. Measure cable outside diameter (or bundle width × thickness), tie width and
   thickness, and tie-head size. The initial model uses a 6 mm round cable and
   a 2.5 × 1 mm tie with a 5 × 5 × 4 mm head envelope; these are assumptions.
...
```bash
uv run --locked python fit-tests/003-cable-retention/build-test.py --output builds/cable-fit
uv run --locked python builds/cable-fit/render-test.py builds/cable-fit
uv run --locked python scripts/check-cable-fit.py --full revisions/009-cable-retention \
  --test builds/cable-fit --output builds/cable-fit/fit-validation.json
```
