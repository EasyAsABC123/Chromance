"""Render revision007 with PCB-clamp and lid heat-set details.
...
def lid_joint(model, revision):
    ...
    preview.render(shapes, revision/"lid-joint.png",
                   ...
                   subtitle="Revision 007 · four joints · M3 × 8 screws · Ø4 × 6 mm blind pilots · simplified hardware",
...
def pcb_joint(model, revision):
    ...
    parts = {
        ...
        "new_PCB_clamp_section": model["case_local_assembly"][name] & half,
...
def replacement_set(model, revision):
    ...
    report.update(status="passed", printed_parts=list(parts),
                  ...
                  note="Body plus four matching PCB clamps only; other ten revision006 parts are reusable.")
    (revision/"replacement-validation.json").write_text(json.dumps(report, indent=2)+"\n")
    preview.PALETTE = ["#59606e"] + ["#9aa5b5"]*4
    preview.render(layout, revision/"replacement-parts.png",
                   title="Revision 007 / five replacement parts",
                   subtitle="Body + four PCB clamps · M3 × 5 inserts and M3 × 6 clamp screws · board-edge slots retained",
...
def main():
    ...
    show(group(parts), revision/"preview.png", "ESP32 / downward-facing buttons",
         "Revision 007 · PCB clamps and lid use heat-set inserts · EN/RESET in blue · BOOT in amber")
    ...
    show(group(parts, lid=False, bracket=False), revision/"interior.png", "ESP32 / inverted internal assembly",
         "Lid removed · four PCB-clamp nut entries removed · board-edge slots retained", elevation=-48)
    ...
    show(group(print_layout(model["parts"])), revision/"print-layout.png", "ESP32 / 15 printable parts",
         "Reprint the body and four PCB clamps · reuse the other ten parts from revision 006",
