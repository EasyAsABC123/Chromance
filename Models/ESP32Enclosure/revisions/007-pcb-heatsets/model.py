"""Revision007: replace the remaining four PCB-clamp nut pockets with heat sets.

The body gains reinforced circular blind pilots and four clamp holes move0.4 mm
outward. The ten other print parts, lid heat sets, desk transform and PCB/button
positions remain unchanged from revision006.
...
def build(params:dict)->dict:
    ...
    mapping={'rotation_x_degrees':180.0,'translation_xyz_mm':[0.,0.,ceiling],
             ...
             'source_frame':'Preserved case coordinates; revision007 adds reinforced PCB heat-set joints to revision006',
    ...
    result['measurements']={
        ...
        'lid_fasteners':{
            ...
            'post_diameter_mm':8.8,'minimum_radial_polymer_mm':4.4-lid_insert_od/2,
    ...
    result['title']='ESP32 / PCB heat-set clamps, no captive-nut pockets · revision007'
    # Older orientation instructions remain in the explicitly local reference only.
    result['reference_notes']=list(result['notes'])
    result['notes']=[
        'Revision007 replaces the four remaining PCB-clamp nut pockets with reinforced M3x5 heat-set joints. Reprint the body and four clamps; the ten other printed parts, including the lid, remain unchanged from006. All older revisions and board-fit tests remain preserved.',
        ...
        f'Lid hardware: four M3x{lid_insert_length:g} heat-set inserts, provisional OD{lid_insert_od:g} mm, and four M3x{p["lid_screw_length"]:g} socket screws. The pilot is diameter{lid_pilot:g} x{lid_depth:g} mm deep; the existing diameter8.8 mm post retains {4.4-lid_insert_od/2:g} mm radial polymer.',
        ...
        'Retain base_model.py, button_module.py and mounting.py beside this wrapper. The base source replaces PCB nut features and reinforces their bosses; button_module.py and mounting.py remain byte-identical to006. Obsolete nut cuts in the overridden source bracket and their helper/parameters are also removed.',
    ]
    return result
