"""Revision010: use the measured8 mm cable with the existing revision009 anchor.

All fifteen printed parts and existing hardware retain revision009 geometry.
Only the cable/tie reference placement and clearance assessment change. The
legacy cable_diameter_provisional parameter key is retained for compatibility;
its default8 mm value is now user supplied. Tie dimensions remain provisional.
"""
...
def _cable_saddle(p, cavity_end_y, sill_z, rim_z):
    ...
    info={
        'status':'Cable diameter supplied by user; physical cable/tie coupon fit pending',
        ...
        'legacy_parameter_note':'cable_diameter_provisional is retained as an input key; default8 mm is measured, tie/head values remain provisional.',
...
def build(params:dict)->dict:
    ...
    result['title']='ESP32 / measured 8 mm cable · revision010'
    # Older orientation instructions remain in the explicitly local reference only.
    result['reference_notes']=list(result['notes'])
    result['notes']=[
        'Revision010 records the user-supplied8 mm cable diameter and verifies it on the existing revision009 support. All fifteen printed parts and34 existing hardware proxies remain unchanged; no replacement print is required for parts already made from009. Earlier revisions and fit tests are preserved.',
        ...
        f'Cable diameter defaults to the user-supplied8 mm measurement; this preset uses{p["cable_diameter_provisional"]:g} mm. The legacy input key remains cable_diameter_provisional for compatibility. Tie/head dimensions and physical fit remain unverified. Thread with lid open and leave internal slack before the termination.',
        f'The existing revision009 saddle adds{cable_info["net_added_volume_mm3"] /1000:.3f} cm3 of CAD solid to the body. No print-time or filament estimate is assigned. The old floor slots are preserved but are unnecessary for this new tie route; its loop does not pass under the desk-facing floor.',
