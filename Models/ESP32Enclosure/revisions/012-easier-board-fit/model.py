"""Revision012: add0.5 mm total fit width and0.5 mm deeper board capture.

The measured PCB remains49x70x1 mm. A separate structural fit-width parameter
moves the shell and shoulders outward while deeper shelves/clamps reference
actual board edges. Button tips stay board-relative; the guide-to-tip arm grows.
Concave cable retention and every height/length datum remain unchanged from011.
"""
...
def build(params:dict)->dict:
    ...
    result['title']='ESP32 / easier board fit and deeper capture shelves · revision012'
    # Older orientation instructions remain in the explicitly local reference only.
    result['reference_notes']=list(result['notes'])
    result['notes']=[
        f'Revision012 adds {p["board_fit_width_extra"]:g} mm total structural width for easier board fitting and extends lower shelves and matching upper clamps by {p["board_edge_overlap"]-0.8:g} mm farther under/over the actual PCB. The measured49 mm substrate width remains unchanged. Body, lid, bracket, four clamps and two sliders need matching replacement geometry; other cartridge pieces retain their shape. Previous revisions are preserved.',
        ...
        f'The lid and extended finger pads face downward when mounted; press upward through {p["button_stroke"]:g} mm nominal travel. Desk contact remains at Z{desk_z:g}, while desk/bracket screw X positions move outward by {p["board_fit_width_extra"]/2:g} mm per side relative to011. Check desk screw spacing before replacing the bracket.',
