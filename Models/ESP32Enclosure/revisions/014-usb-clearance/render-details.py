"""Render actual revision 014 geometry and illustrative cable/tie routing."""
...
def render(shapes,path,title,subtitle,elevation=30,azimuth=-65):
    palette={'body':'#59606e','lid':'#9aa5b5','desk_bracket':'#454c56',
             'shell_section':'#798391','concave_cable_cradle':'#55aaba',
...
def main():
    ...
    render(grouped(parts),output/'preview.png','ESP32 / measured USB clearance',
           'Revision 014 · Measured USB plug · full-stroke slider and rail relief',-25,-145)
    scene=grouped(parts)
    ...
    render(grouped(parts,lid=False,bracket=False),output/'interior.png','Measured USB relief / lid removed',
           'Actual assembled geometry · revised sliders and BOOT frame',-48,-145)
    ...
    render(grouped(exploded),output/'exploded.png','Enclosure / assembly access',
           'Lid lowered for illustration · matching shell, clamps and sliders required',-25,-145)
    render(grouped(print_layout(model['parts'])),output/'print-layout.png','Revision 014 / 15 printable parts',
           'Three replacement pieces; verify the USB fit and button travel with test009',52,-65)
    ...
    section={'shell_section':body-feature,'concave_cable_cradle':feature}
    render(section,output/'cable-anchor.png','Cable anchor / actual enclosure section',
           'Concave cylindrical cut · 2 mm walls/floor · zip-tie passage underneath',32,-65)
