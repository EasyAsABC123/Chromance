"""Revision 014: measured USB relief in the two sliders and BOOT frame.

The conservative measured plug envelope is relieved through full button travel.
Stationary rail relief opens upward; insert walls and board datums are retained.
Board retention, cable cradle and all twelve other printed parts are preserved.
"""
...
def build(params:dict)->dict:
    ...
    for name,data in result['buttons'].items():
        ...
        if _overlap(extra,result['usb_relief_references_local'][name+'_slider_relief_cutter'])>1e-7:
            raise ValueError('USB swept relief reaches the pad extension; revise its geometry explicitly')
    ...
    mapping={'rotation_x_degrees':180.0,'translation_xyz_mm':[0.,0.,ceiling],
             ...
             'source_frame':'Case coordinates: measured PCB separate from structural fit width; deeper shelves/clamps; PCB top Z10 and concave cable cradle retained',
    ...
    result['measurements']={
        ...
        'pcb_fasteners':{
            ...
            'screw_offset_from_measured_pcb_edge_mm':p['pcb_clamp_screw_offset_from_edge']+p['board_fit_width_extra']/2,
            'screw_offset_from_structural_fit_edge_mm':p['pcb_clamp_screw_offset_from_edge'],
            'screw_center_outward_shift_from_011_mm':p['board_fit_width_extra']/2,
        ...
        'nut_trap_inventory':{
            ...
            'desk_bracket':'Four blind heat-set pilots; bracket follows the independently adjustable structural fit width',
        ...
        'board_retention':deepcopy(original_measurements['board_retention']),
    ...
    result['measurements']['button_alignment'] = {
        'status': 'User-supplied XY implemented; physical fit and switch height/travel pending',
        'datum': 'Centered 49x70 PCB; left edge X=-width/2, bottom edge Y=-length/2; local Z toward lid',
        'left_edge_inset_mm': p['button_inset_from_left_pcb_edge'],
        'reset_from_bottom_mm': p['reset_button_from_bottom'],
        'boot_from_bottom_mm': p['boot_button_from_bottom'],
        'tip_spacing_y_mm': p['reset_button_from_bottom']-p['boot_button_from_bottom'],
        'tips_local_xy_mm': {name:data['measurements']['tip_center_xy_mm'] for name,data in result['buttons'].items()},
        'housing_centers_y_mm': [p['reset_housing_y'],p['boot_housing_y']],
        'lower_collar_gap_y_mm': p['reset_button_from_bottom']-p['boot_button_from_bottom']-2*p['button_contact_collar_flat_offset'],
        'measurement_record': 'measurements/2026-09-12-button-alignment.json',
        'fit_test': 'fit-tests/009-usb-clearance',
    }
    result['measurements']['usb_clearance'] = deepcopy(original_measurements['usb_clearance'])
    result['title']='ESP32 / measured USB clearance and button travel · revision 014'
    ...
    result['notes']=[
        ...
        f'Additional total fit width is {p["board_fit_width_extra"]:g} mm. The shoulders are {original_measurements["board_retention"]["shoulder_gap_mm"]:g} mm apart, giving {original_measurements["board_retention"]["centered_side_clearance_each_mm"]:g} mm centered clearance per side. Both shelves and top clamps overlap the centered measured board by {p["board_edge_overlap"]:g} mm and retain at least {original_measurements["board_retention"]["minimum_overlap_after_lateral_play_mm"]:g} mm after full lateral play. The vertical capture slot remains {p["pcb_thickness"]+p["board_vertical_play"]:g} mm high.',
        f'Button contact centers are {p["reset_button_from_bottom"]:g}/{p["boot_button_from_bottom"]:g} mm from the PCB bottom edge and {p["button_inset_from_left_pcb_edge"]:g} mm from the left edge. These are user-supplied XY dimensions at nominal centered board placement; actual height, switch travel, PCB lateral play and physical fit require the button coupon.',
        ...
        f'Normalized original USB chart A/B/C/D/E/F is {p["usb_plug_width"]:g}/{p["usb_plug_underside_above_pcb"]+p["usb_plug_thickness"]:g}/{p["usb_plug_projection"]:g}/{p["usb_plug_underside_above_pcb"]:g}/{p["usb_center_from_bottom"]:g}/{p["usb_cable_diameter"]:g} mm: width, top above PCB, outboard projection, underside above PCB, center from bottom and USB cable diameter. The user clarified raw B6 as thickness6; original diagram B is derived top15 (D9 + thickness6). Active plug thickness is {p["usb_plug_thickness"]:g} mm. The measured USB cable is separate from the 8 mm end-cradle cable.',
        f'Provisional USB relief clearance is {p["usb_fit_clearance"]:g} mm. Slider relief covers the stationary expanded plug translated upward through the full {p["button_stroke"]:g} mm stroke and opens below the slider so it can top-load around an installed plug. The affected BOOT forward rail opens upward without a thin top lip. Minimum remaining material to the contact insert envelope is {original_measurements["usb_clearance"]["minimum_wall_to_insert_envelope_mm"]:.3f} mm. Maximum plug width is conservatively applied across its full projection; strain-relief taper and cable axis height remain unmeasured.',
        ...
        f'The lid and extended finger pads face downward when mounted; press upward through {p["button_stroke"]:g} mm nominal travel. Desk contact remains at Z{desk_z:g}; the bracket and all mounting positions are unchanged from013.',
