"""Revision006 base: revision005 geometry with heat-set lid-post drilling.
...
def build_console(params: dict) -> dict:
    ...
    # Revision006 lid fasteners: preserve the original Ø8.8 posts and lid.
    # Only the post drilling changes; the former hex pockets/entries are absent.
    ...
    if 4.4 - p["lid_insert_outer_diameter"] / 2 < 2.0 - 1e-8:
        raise ValueError("Lid insert requires at least 2 mm radial polymer in the existing post")
    ...
    # Lid screw posts sit beyond the PCB ends. Heat-set inserts install from
    # the open rim before the lid is fitted; no hex pocket or side entry remains.
    if h - p["lid_insert_bore_depth"] < floor + 2:
        raise ValueError("Lid pilot must leave at least 2 mm of post above the floor")
    for x, y in lid_points:
        body += _cylinder(4.4, h - floor, x, y, floor)
        body -= _cylinder(p["lid_insert_pilot_diameter"] / 2,
                          p["lid_insert_bore_depth"] + 0.1,
                          x, y, h - p["lid_insert_bore_depth"])
    ...
    return {
        ...
        "notes": [
            ...
            f"Base-source lid hardware: 4 M3x{p['lid_screw_length']:g} screws and 4 M3x{p['lid_insert_length']:g} heat-set inserts. PCB clamps retain their original M3x8 screws and hex nuts. The revision006 wrapper defines the actual desk bracket hardware.",
