"""Freeze all local CAD modules before exporting a fresh button revision."""
import argparse
import hashlib
import json
...
def main():
    ...
    sources = ["model.py", "button_module.py", "mounting.py", "build-revision.py", "render-details.py"]
    with tempfile.TemporaryDirectory(prefix="button-sources-") as tmp:
        snapshot = Path(tmp)
        for name in sources:
