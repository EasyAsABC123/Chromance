"""Independent geometry/service review of revision004 heatset cartridges.
...
def case(label, model_dir, baseline_path, prior_module_path, params):
    result=load_model(model_dir/'model.py',params)
    bm=module(model_dir/'button_module.py','button_module_review')
    prior_module=module(prior_module_path,'prior_cartridge_review')
    ...
    prior_params={k:value for k,value in params.items() if not k.startswith('button_contact_insert_')}
    prior=load_model(prior_module_path.parent/'model.py',prior_params)
    for n,shape in prior['assembly'].items():
        if n not in ('reset_slider','boot_slider'):
            same('exact revision003 unchanged part '+n,result['assembly'][n],shape)
    ...
    for fraction in [0.,.5,1.]:
        ...
        hw_found=[]
        allowed = {(entry['hardware'], entry['printed']): entry for button in buttons.values() for entry in button.get('expected_hardware_intersections', [])}
        expected_pairs = {(name+'_contact_heatset_insert', name+'_slider') for name in buttons}
        check(pose+' only insert and own host may displace polymer',int(set(allowed) != expected_pairs),0)
        for hn,hs in hardware.items():
            for n,s in assembly.items():
                overlap=v(common(hs,s))
                if (hn,n) in allowed:
                    check(pose+' '+hn+' exact displaced annulus volume',overlap-allowed[(hn,n)]['nominal_displaced_envelope_volume_mm3'])
                elif overlap>TOL:hw_found.append({'hardware':hn,'part':n,'volume_mm3':overlap})
    ...
    # Revision004 must change only each distal contact boss/hardware.
    for name,bdata in result['buttons'].items():
        bp=bdata['params']; candidate=bm.make_button(bp,name)
        old_bp={k:value for k,value in bp.items() if k in prior_module.DEFAULTS}
        old=prior_module.make_button(old_bp,name)
        tx,ty=bp['tip_x'],bp['tip_y'];a=bp['arm_bottom_z']
        for suffix in ['frame','rear_keeper','moving_magnet_keeper']:
            same(name+' exact unchanged '+suffix,candidate['assembly'][name+'_'+suffix],old['assembly'][name+'_'+suffix])
        region=cyl(4.6,40,tx,ty,0)
        slider=candidate['assembly'][name+'_slider']
        same(name+' slider unchanged outside contact boss',slider-region,old['assembly'][name+'_slider']-region)
        for key in ['fixed_magnet','moving_magnet','mount_screw_1','mount_screw_2']:
            same(name+' exact unchanged hardware '+key,candidate['hardware'][name+'_'+key],old['hardware'][name+'_'+key])
        mm=candidate['measurements'];bottom=mm['contact_boss_bottom_z_at_rest_mm'];topboss=mm['contact_boss_top_z_at_rest_mm']
        depth=mm['contact_insert_pocket_depth_mm'];pilot=mm['contact_insert_pilot_diameter_provisional_mm']
        check(name+' pocket depth equals insert length plus0.4',depth-bp['contact_insert_length']-.4,1e-6)
        check(name+' annular floor at least1mm',max(0,1-(topboss-depth-bottom)),1e-6)
        empty(name+' pilot bore clear',cyl(pilot/2-.01,depth-.02,tx,ty,topboss-depth+.01),{'slider':slider})
        # Narrow floor and outer radial-wall probes avoid coincident faces.
        floor_ring=cyl(pilot/2-.02,.1,tx,ty,bottom+.1)-cyl(1.72,.2,tx,ty,bottom+.05)
        check(name+' annular floor contains polymer',v(floor_ring)-v(common(floor_ring,slider)))
        old_feed=box(.2,.2,.2,tx+3.8,ty,a+1.2)
        check(name+' former lateral nut feed filled',v(old_feed)-v(common(old_feed,slider)))
        check(name+' no contact captive nut remains',int(name+'_nylon_captive_nut' in candidate['hardware']),0)
        check(name+' head clears nominal lid underside',max(0,mm['contact_head_top_z_at_rest_mm']-h),1e-6)
        check(name+' adjustment margin at least0.19mm',max(0,.19-mm['contact_downward_adjustment_margin_mm']),1e-6)
        insert=candidate['hardware'][name+'_contact_heatset_insert']
        # Once heated into its host, insert accompanies slider in the top-load path.
        check(name+' inserted metal top-load path clear',max(v(common(Pos(0,0,dz)*insert,candidate['assembly'][name+'_frame'])) for dz in [.5,1,2,4,8,16,32]))
        # Body attachment insert bores, shell, lid and bracket are unchanged from003.
...
def main():
    ...
    parser.add_argument('--baseline',type=Path,required=True,help='Revision002 console model.py')
    parser.add_argument('--prior-module',type=Path,required=True,help='Revision003 button_module.py')
    parser.add_argument('--cases',nargs='+',choices=['default','smaller_stroke_wider_guide','larger_board','lower_switch_longer_contact'])
    args=parser.parse_args()
    parameter_path=args.model_dir/'params.json'
    if not parameter_path.exists():parameter_path=args.model_dir/'parameters.json'
    params=json.loads(parameter_path.read_text())
    args.output.parent.mkdir(parents=True,exist_ok=True)
    result={'status':'running','inputs':{},'review_sources':{},'cases':[]}
    for key,path in {'checker':Path(__file__),'baseline_model':args.baseline,
                     'prior_module':args.prior_module,'prior_model':args.prior_module.parent/'model.py'}.items():
        result['review_sources'][key]={'path':str(path.resolve()),'sha256':hashlib.sha256(path.read_bytes()).hexdigest()}
    for f in ['model.py','button_module.py','mounting.py',parameter_path.name]:
    ...
    for name,changes in variants:
        ...
        c=case(name,args.model_dir,args.baseline,args.prior_module,params|changes);result['cases'].append(c)
    ...
    result['inputs_unchanged']=all(hashlib.sha256((args.model_dir/f).read_bytes()).hexdigest()==details['sha256'] for f,details in result['inputs'].items())
    result['review_sources_unchanged']=all(hashlib.sha256(Path(details['path']).read_bytes()).hexdigest()==details['sha256'] for details in result['review_sources'].values())
    result['status']='passed' if result['inputs_unchanged'] and result['review_sources_unchanged'] and all(c['passed'] for c in result['cases']) else 'failed'
    args.output.write_text(json.dumps(result,indent=2)+'\n')
