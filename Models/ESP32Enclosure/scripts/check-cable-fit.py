...
def main():
    ...
    report={'status':'passed','kind':'Source identity and independent STEP crop comparison','source_revision':'009-cable-retention','matched_source_hashes':matched,'parameters_sha256':digest(test/'parameters.json'),'checker_sha256':digest(__file__),'full_step_hashes':{name:digest(full/'parts'/f'{name}.step') for name in ('body','lid')},'parts':details,'full_anchor_region_preserved':True,'print_orientation_matches_full_parts':True,'physical_fit_tested':False,'physical_strength_tested':False,'scope':'Local threading, seating and lid-clearance coupon; full-shell stiffness and wiring/board fit remain outside this crop.'}
